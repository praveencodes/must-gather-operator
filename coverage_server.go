...
// coverageEnabled is true when the binary is built with the coverage build tag.
const coverageEnabled = true

// startCoverageServer starts the HTTP server for coverage collection in a
// separate goroutine. Called from main when coverageEnabled is true.
