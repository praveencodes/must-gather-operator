...
## Execution agent roster (OpenSpec bugfix workflow)

| Agent ID | Owns |
|----------|------|
| `API_Agent` | `api/v1alpha1/**`, CRD generation/manifests |
| `OperatorController_Agent` | `controllers/mustgather/mustgather_controller.go`, predicates, status/finalizers |
| `JobTemplate_Agent` | `controllers/mustgather/template.go`, `build/bin/upload` |
| `Testing_Agent` | `controllers/mustgather/*_test.go`, `make go-test`, e2e targets |

Assigned Agent values in `tasks.md` MUST match IDs defined here.

## Baseline note for OCPBUGS-63242

Working tree is pinned at `0ff4e004fabb57afc9aa26e93639f2674e04bace` for this bugfix. Agentic docs above are sourced from [PR #379](https://github.com/openshift/must-gather-operator/pull/379) (`OAPE-881`) for routing/context. `harness-evals/` may be absent from the baseline checkout — copies used for this change live under `openspec/changes/ocpbugs-63242/inputs/harness-docs/`. Prefer baseline controller code over later commits when analyzing this bug.

### Per-task verification matrix

| Task type | Verify with |
|-----------|-------------|
| Code Fix (controller/API) | `go test ./controllers/mustgather/...` and/or `make go-test` |
| Job template change | `go test ./controllers/mustgather/ -run Template` |
| Regression Test | targeted `go test` for new cases + package tests |
| Verification | `make go-test` |
